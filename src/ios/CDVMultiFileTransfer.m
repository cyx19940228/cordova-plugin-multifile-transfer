...
@interface CDVMultiFileTransfer ()
// Sets the requests headers for the request.
- (void)applyRequestHeaders:(NSDictionary*)headers toRequest:(NSMutableURLRequest*)req;
// Creates a delegate to handle an upload.
- (CDVMultiFileTransferDelegate*)delegateForUploadCommand:(CDVInvokedUrlCommand*)command;
...
@implementation CDVMultiFileTransfer
...
- (NSURLRequest*)requestForUploadCommand:(CDVInvokedUrlCommand*)command fileData:(NSArray *)fileDatas
{
    ...
    BOOL chunkedMode = [[command argumentAtIndex:7 withDefault:[NSNumber numberWithBool:NO]] boolValue];
    ...
    CDVMultiFileTransferError errorCode = 0;
...
//            }];
    ...
    long long totalPayloadLength = 0;
    ...
    if (multipartFormUpload) {
        ...
        totalPayloadLength += [postBodyBeforeFile length];
    }
...
- (CDVMultiFileTransferDelegate*)delegateForUploadCommand:(CDVInvokedUrlCommand*)command
{
    ...
    CDVMultiFileTransferDelegate* delegate = [[CDVMultiFileTransferDelegate alloc] init];
...
- (void)fileDataForUploadCommand:(CDVInvokedUrlCommand*)command
{
    ...
    NSArray *sources = (NSArray *)[command argumentAtIndex:0];
    NSString* server = [command argumentAtIndex:1];
    ...
    for (NSString *source in sources) {
        ...
        if (fs) {
            __weak CDVMultiFileTransfer* weakSelf = self;
    ...
    [self uploadData:fileDatas command:command];
}
...
- (void)uploadData:(NSArray *)fileDatas command:(CDVInvokedUrlCommand*)command
{
    NSURLRequest* req = [self requestForUploadCommand:command fileData:fileDatas];
    
    if (req == nil) {
        return;
    }
    CDVMultiFileTransferDelegate* delegate = [self delegateForUploadCommand:command];
...
- (void)abort:(CDVInvokedUrlCommand*)command
{
    ...
    @synchronized (activeTransfers) {
        CDVMultiFileTransferDelegate* delegate = activeTransfers[objectId];
        if (delegate != nil) {
            [delegate cancelTransfer:delegate.connection];
...
- (void)onReset {
    @synchronized (activeTransfers) {
        while ([activeTransfers count] > 0) {
            CDVMultiFileTransferDelegate* delegate = [activeTransfers allValues][0];
            [delegate cancelTransfer:delegate.connection];
        }
    }
}
...
@interface CDVMultiFileTransferEntityLengthRequest : NSObject {
    NSURLConnection* _connection;
    CDVMultiFileTransferDelegate* __weak _originalDelegate;
}

- (CDVMultiFileTransferEntityLengthRequest*)initWithOriginalRequest:(NSURLRequest*)originalRequest andDelegate:(CDVMultiFileTransferDelegate*)originalDelegate;

@end

@implementation CDVMultiFileTransferEntityLengthRequest
...
@implementation CDVMultiFileTransferDelegate
...
- (void)connectionDidFinishLoading:(NSURLConnection*)connection
{
    ...
    if (self.direction == CDV_TRANSFER_DOWNLOAD) {
        if (self.targetFileHandle) {
            ...
            CDVMultiFileTransferError errorCode = self.responseCode == 404 ? FILE_NOT_FOUND_ERR
...
- (void)cancelTransfer:(NSURLConnection*)connection
{
    ...
    @synchronized (self.command.activeTransfers) {
        CDVMultiFileTransferDelegate* delegate = self.command.activeTransfers[self.objectId];
...
- (void)connection:(NSURLConnection*)connection didReceiveResponse:(NSURLResponse*)response
{
    ...
    if ([response isKindOfClass:[NSHTTPURLResponse class]]) {
        ...
        if ((self.direction == CDV_TRANSFER_DOWNLOAD) && (self.responseCode == 200) && (self.bytesExpected == NSURLResponseUnknownLength)) {
            ...
            self.entityLengthRequest = [[CDVMultiFileTransferEntityLengthRequest alloc] initWithOriginalRequest:connection.currentRequest andDelegate:self];
        }
